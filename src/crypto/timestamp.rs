...
/// Context-specific constructed [3]: `extensions` in a tbsCertificate.
...
const OID_KP_TIMESTAMPING: [u8; 8] = [0x2b, 0x06, 0x01, 0x05, 0x05, 0x07, 0x03, 0x08];
...
pub enum TimestampError {
    ...
    #[error(
        "the certificate that signed this token is not authorised to timestamp \
         (RFC 3161 requires the critical id-kp-timeStamping extended key usage)"
    )]
...
fn parse_utc_time(raw: &[u8]) -> Result<DateTime<Utc>, TimestampError> {
    ...
    // len() counts bytes but the slicing below is by byte index, so a
    // multi-byte character straddling index 2 would be a char boundary
    // violation -- a panic driven by whoever supplied the token. UTCTime is
    // YYMMDDHHMMSS, so demanding digits is both correct and sufficient.
    if stripped.len() != 12 || !stripped.bytes().all(|b| b.is_ascii_digit()) {
...
fn oid_to_dotted(raw: &[u8]) -> Result<String, TimestampError> {
    ...
    let mut value: u64 = 0;
    for &byte in rest {
        // Not checked_shl: that reports a shift wider than the type, never
        // the high bits a legal shift discards. Seven bits move out of the
        // top on each step, so the top seven must be clear beforehand.
        if value >> 57 != 0 {
            return Err(TimestampError::Malformed("OID arc overflows"));
        }
...
pub fn verify_token(
    ...
    // Holding a certificate from the pinned issuer is not the same as being
    // allowed to timestamp with it.
    if !signer.allows_timestamping {
...
/// The fields of an X.509 certificate this verifier reads. Extensions are
/// read only far enough to answer one question -- is this certificate
/// allowed to timestamp -- because the pinned issuer vouches for who signed,
/// not for what they were authorised to sign. Policies, name constraints and
/// the rest are deliberately not interpreted; see `verify_token` on why no
/// path is built here.
struct Certificate<'a> {
    ...
    /// Whether the certificate carries the extended key usage RFC 3161 §2.3
    /// requires of a timestamping authority.
    ///
    /// Without this, any certificate the pinned issuer ever signed would do
    /// -- a TLS server certificate from the same CA could stamp time. The
    /// pinned issuer being a dedicated timestamping CA is an assumption
    /// about someone else's operations, not something a verifier can check,
    /// so the leaf's own authorisation is checked instead.
    allows_timestamping: bool,
}
...
impl<'a> Certificate<'a> {
    fn parse(der: &'a [u8]) -> Result<Self, TimestampError> {
        ...
        // What remains of a tbsCertificate is issuerUniqueID [1],
        // subjectUniqueID [2] and extensions [3], each optional. Step over
        // anything that is not [3].
        let mut allows_timestamping = false;
        while let Some(tag) = fields.peek() {
            if tag != TAG_CONTEXT_3 {
                fields.skip_any()?;
                continue;
            }
            let explicit = fields.take(TAG_CONTEXT_3)?;
            let mut extensions = Der::new(explicit).sequence()?;
            while let Ok(mut extension) = extensions.sequence() {
                let id = extension.oid()?;
                // `critical` is a DEFAULT FALSE BOOLEAN, so it may be absent.
                let mut critical = false;
                if extension.peek() == Some(TAG_BOOLEAN) {
                    critical = extension
                        .take(TAG_BOOLEAN)?
                        .first()
                        .is_some_and(|b| *b != 0);
                }
                let value = extension.octet_string()?;
                if id != OID_EXT_KEY_USAGE {
                    continue;
                }
                // RFC 3161 §2.3: the certificate MUST contain exactly one
                // extended key usage, id-kp-timeStamping, and the extension
                // MUST be critical. Both halves matter. A non-critical usage
                // is advisory, so software that ignores it would accept the
                // certificate for anything; and a certificate authorised for
                // several purposes is one whose other purposes could be used
                // to obtain a signature that reads as a timestamp. This is
                // also what separates a responder from the CA above it: the
                // issuing CA carries the same usage non-critically, to
                // constrain what it may issue, and must not itself stamp.
                let mut purposes = Der::new(value).sequence()?;
                let mut found = Vec::new();
                while let Ok(purpose) = purposes.oid() {
                    found.push(purpose);
                }
                allows_timestamping =
                    critical && found.len() == 1 && found[0] == OID_KP_TIMESTAMPING;
            }
            break;
        }
...
/// Whether a certificate is authorised to sign timestamp tokens.
///
/// Exposed so the property can be tested directly: a CA certificate and a
/// responder certificate differ here, and nothing else in a token makes the
/// difference visible.
pub fn certificate_allows_timestamping(der: &[u8]) -> Result<bool, TimestampError> {
    Ok(Certificate::parse(der)?.allows_timestamping)
}
...
mod adversarial {
    ...
    /// A UTCTime whose bytes are valid UTF-8 but not ASCII must be refused,
    /// not panicked on. `len()` counts bytes; `[..2]` slices a &str at a
    /// byte index, so a multi-byte character straddling index 2 is a char
    /// boundary violation. The bytes come from a certificate inside a token
    /// supplied by whoever answers the timestamp request.
    #[test]
    fn a_utctime_with_a_multibyte_character_is_refused() {
        // '€' is three bytes, so 3 + 9 ASCII digits = 12 bytes before 'Z'
        // and byte index 2 lands inside the character.
        let raw = "€260101000Z".as_bytes();
        assert_eq!(raw.len() - 1, 12, "fixture must reach the length check");
        assert!(parse_utc_time(raw).is_err());
    }

    /// The same class of input, one layer up: a certificate validity field
    /// is read through `Der::time`, so the refusal has to hold there too.
    #[test]
    fn a_multibyte_time_inside_a_tlv_is_refused() {
        let body = "€260101000Z".as_bytes();
        let mut tlv = vec![TAG_UTC_TIME, body.len() as u8];
        tlv.extend_from_slice(body);
        assert!(Der::new(&tlv).time().is_err());
    }

    /// An OID arc wider than 64 bits must be reported, not silently
    /// truncated into a different OID than the one on the wire.
    #[test]
    fn an_oid_arc_wider_than_the_accumulator_is_refused() {
        // 0x2a, then ten continuation bytes carrying 70 bits of value.
        let mut raw = vec![0x2a];
        raw.extend(std::iter::repeat_n(0xffu8, 10));
        raw.push(0x7f);
        assert!(oid_to_dotted(&raw).is_err());
    }

    /// A well-formed OID still decodes.
    #[test]
    fn an_ordinary_oid_still_decodes() {
        assert_eq!(
            oid_to_dotted(&OID_TST_INFO).unwrap(),
