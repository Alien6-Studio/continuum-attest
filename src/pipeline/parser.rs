...
pub struct Pipeline {
    ...
    /// BTreeMap, not HashMap: `pipeline_hash` is blake3 of this struct's
    /// serialization, and a HashMap iterates in an order Rust randomizes per
    /// process. The hash was therefore not a function of the pipeline -- two
    /// identical runs produced different pipeline hashes, and a verifier
    /// comparing them failed on honest receipts. An ordered map makes the
