...
- **The causal root is never published outside the machine that made it.** The
  ledger is append-only by construction, locally. There is no external proof of
  that property — no transparency log, no anchor.
- **The signer is a public key, not a person.** An Ed25519 key in a trust store
  has no binding to a legal identity.

**Time** used to be a third. Without `--timestamp`, a receipt's timestamp is
written by the runner and covered by the runner's own signature: it asserts
nothing to anyone who does not already trust the runner. That also weakened
revocation, which compares against it — whoever stole a key could backdate a
receipt past the revocation meant to stop them. With `--timestamp`, the time
comes from an authority the signer does not control, revocation is judged
against it, and `attest verify` says so when it falls back.
...
- [Plugin protocol](https://attest.continuu.ms/en/docs/plugin-protocol/) — the plugin boundary.
