...
fn test_cache_key_handles_binary_inputs() -> Result<()> {
    ...
    let key1 = storage.compute_content_hash(
        workspace.path(),
        "step",
        "cat blob.bin",
        &inputs,
    ...
    let key2 = storage.compute_content_hash(
        workspace.path(),
        "step",
        "cat blob.bin",
        &inputs,
    ...
    let key3 = storage.compute_content_hash(
        workspace.path(),
        "step",
        "cat blob.bin",
        &inputs,
...
fn test_cache_key_changes_when_input_removed() -> Result<()> {
    ...
    let key_with_input = storage.compute_content_hash(
        workspace.path(),
        "step",
        "cmd",
        &inputs,
    ...
    std::fs::write(&file, "data")?;
    let key_again = storage.compute_content_hash(
        workspace.path(),
        "step",
        "cmd",
        &inputs,
...
fn test_cache_key_tracks_manifest_input_hash() -> Result<()> {
    ...
    let key_v1 =
        storage.compute_content_hash(workspace.path(), "step", "cmd", &inputs, &env, "shape")?;
