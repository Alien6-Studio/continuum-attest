...
"#,
    ...
    pub fn compute_content_hash(
        ...
        let mut hasher = blake3::Hasher::new();
        // v4: the key covers the step's whole shape, not only its
        // command, inputs and environment. Bumping the domain string retires
        // every v1 entry rather than letting one be served under the new
        // rules, which is the point of having a domain string at all.
        hasher.update(b"attest-cache-key/v4\n");
        // Every field length-prefixed. Newline-separated `key=value` lines
        // let a value containing a newline forge a variable, so two
        // different environments hashed alike and one run was served the
        // other's result -- then signed into a receipt describing the run
        // that did not happen. Step names have the same freedom.
        crate::hashing::hash_field(&mut hasher, "shape", step_shape.as_bytes());
        crate::hashing::hash_field(&mut hasher, "step", step_name.as_bytes());
        crate::hashing::hash_field(&mut hasher, "inputs", input_hash.as_bytes());
        ...
        for (key, value) in env_entries {
            crate::hashing::hash_field(&mut hasher, "env-key", key.as_bytes());
    ...
    pub fn create_cache_entry(
        ...
        // Same reason as above, and worse here: there was no separator at
        // all, so {"AB": "C"} and {"A": "BC"} produced identical bytes.
        ...
        crate::hashing::hash_field(
            &mut env_hasher,
            "env-count",
            env_sorted.len().to_string().as_bytes(),
        );
        for (key, value) in env_sorted {
            crate::hashing::hash_field(&mut env_hasher, "env-key", key.as_bytes());
            crate::hashing::hash_field(&mut env_hasher, "env-value", value.as_bytes());
        }
