...
//! bind both the file contents and the exact command. That command is
//! recorded as a JSON array rather than a space-joined string, because
//! joining loses the boundaries between arguments and two different
//! executions would then share every hash a receipt records.
...
pub fn run_wrap(command: &[String], options: &WrapOptions) -> Result<i32> {
    ...
    // The argument vector is hashed and attested, so its boundaries have to
    // survive the encoding. Joining with spaces made `echo "a b"` and
    // `echo a b` indistinguishable: identical command hash, identical input
    // hash, different execution. A JSON array is unambiguous and still
    // readable in a receipt.
    let run_string = serde_json::to_string(command).context("cannot encode the wrapped command")?;
    let mut provenance =
