...
/// Feed one labelled field into a hasher, unambiguously.
///
/// Concatenating `key=value` pairs, or joining anything with a separator,
/// makes the hash a function of the concatenation rather than of the fields.
/// A value containing the separator then forges a field: `ZZ_A` holding
/// `one\nZZ_B=two` produced the same bytes as two separate variables, so two
/// different environments shared a cache key and one run was served the
/// other's result. The same shape had already been found in the manifest
/// encoding and in causal event identity; this exists so the next place that
/// needs to hash structured data does not have to rediscover it.
///
/// The length prefix is what makes the encoding injective: no value can
/// masquerade as the end of its own field.
pub fn hash_field(hasher: &mut blake3::Hasher, label: &str, value: &[u8]) {
    // Every length precedes what it measures. That ordering is the whole
    // property: a reader takes the digits, then exactly that many bytes, and
    // nothing inside those bytes can be mistaken for a boundary.
    //
    // The first version of this function wrote the label before its own
    // length, and the label could then absorb the markers: label "a" with
    // value "z:7:0:" encoded identically to label "a:1:6:z" with an empty
    // value. Nothing in the product could reach it, since every call site
    // passes a constant label -- which is precisely why it survived review
    // and needed a test.
    ...
    for component in rel.components() {
        match component {
            Component::Normal(part) => {
                ...
                // A manifest is a line-delimited document and paths are
                // written into it verbatim. A component carrying a newline
                // would close its own entry and open a forged one, so two
                // different trees could hash identically -- integrity lost
                // without touching BLAKE3. Refusing control characters is
                // what keeps the encoding injective. Rejecting rather than
                // escaping is deliberate: every path that could be hashed
                // before still hashes to the same value, so receipts written
                // by earlier versions stay verifiable.
                if let Some(bad) = part.chars().find(|c| c.is_control()) {
                    return Err(HashingError::InvalidPath {
                        path: abs.to_path_buf(),
                        reason: format!(
                            "path component contains the control character U+{:04X}, \
                             which a manifest cannot represent unambiguously",
                            bad as u32
                        ),
                    });
                }
