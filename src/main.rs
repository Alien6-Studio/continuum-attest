...
#[derive(Subcommand)]
enum Commands {
    /// Connect this workspace to optional SaaS receipt delivery (includes captured logs)
    Connect {
        #[arg(long, default_value = "https://app.attest.continuu.ms")]
        endpoint: String,
        #[arg(long)]
        organization: String,
    ...
    /// Disable automatic delivery for this workspace; local receipts remain available
    Disconnect,
    /// Queue and deliver an existing signed receipt to the configured SaaS
    Publish { receipt: std::path::PathBuf },
    /// Retry queued receipts for this workspace
    Sync,
    ...
    Verify {
        ...
        #[arg(long, help = "Recompute input hashes from the workspace pipeline")]
        recompute: bool,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Connect {
            ...
            attest::sync::connect(&std::env::current_dir()?, &endpoint, &organization)?;
            Ok(())
        }
        Commands::Disconnect => attest::sync::disconnect(&std::env::current_dir()?),
        Commands::Publish { receipt } => {
            let workspace = std::env::current_dir()?;
            std::thread::spawn(move || attest::sync::publish(&workspace, &receipt))
                .join()
                .map_err(|_| anyhow::anyhow!("delivery worker failed"))?
        }
        Commands::Sync => {
            let workspace = std::env::current_dir()?;
            std::thread::spawn(move || attest::sync::flush(&workspace, 100))
                .join()
                .map_err(|_| anyhow::anyhow!("delivery worker failed"))?
        }
        ...
        Commands::Import {
            ...
            let code = handle_import(statement, format, workspace, trust_store, report);
            std::process::exit(code);
        }
